...
use xrpl_common_stdlib::objects::{AccountRoot, cache_ledger_entry};
...
pub extern "C" fn escrow_finish() -> i32 {
    ...
    // ########################################
    {
        ...
        let slot = cache_ledger_entry(&accountroot_id).unwrap_or_else(|error| {
            let _ = trace_num("Error slotting Account object", error.code() as i64);
            panic!()
        });
