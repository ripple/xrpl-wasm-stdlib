...
pub fn expand(input: TokenStream) -> TokenStream {
    ...
    match decode_classic_address_to_20bytes(&addr) {
        ...
        Err(reason) => syn::Error::new(addr_lit.span(), format!("Invalid r-address: {reason}"))
            .to_compile_error()
            .into(),
    }
}
...
mod tests {
    ...
    fn rejects_missing_r_prefix() {
        ...
        assert!(decode_classic_address_to_20bytes("xHb9CJAWyB4rj91VRWn96DkukG4bwdtyTh").is_err());
    }

    #[test]
    fn rejects_bad_checksum() {
        // Last character flipped — payload is fine, checksum no longer matches.
        assert!(decode_classic_address_to_20bytes("rHb9CJAWyB4rj91VRWn96DkukG4bwdtyTa").is_err());
    }

    #[test]
    fn rejects_too_short() {
        assert!(decode_classic_address_to_20bytes("rTooShort").is_err());
    }
    ...
    fn rejects_chars_outside_xrpl_alphabet() {
        ...
        assert!(decode_classic_address_to_20bytes("r0b9CJAWyB4rj91VRWn96DkukG4bwdtyTh").is_err());
    }

    #[test]
    fn rejects_empty() {
        assert!(decode_classic_address_to_20bytes("").is_err());
    }

    #[test]
    fn rejects_bare_r() {
        assert!(decode_classic_address_to_20bytes("r").is_err());
    }
}
