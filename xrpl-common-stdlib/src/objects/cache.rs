...
//! of its cache slots. [`cache_le`] is the second step, and the slot it returns is what every
//! slot-based handle is built from — [`LedgerObject::new`](crate::objects::LedgerObject::new) for
//! an object with no typed wrapper, `<Entry>::new` for one that has it.
...
mod tests {
    ...
    fn test_returns_the_slot_the_host_assigned() {
        ...
        assert_eq!(cache_le(&[0xAB; 32]).unwrap(), 4);
    }
    ...
    fn test_reports_a_missing_entry_as_an_error() {
        ...
        assert_eq!(
            cache_le(&[0x00; 32]).err().unwrap().code(),
    ...
    fn test_reports_an_exhausted_cache_as_an_error() {
        ...
        assert_eq!(cache_le(&[0x11; 32]).err().unwrap().code(), SLOTS_FULL);
    }
}
