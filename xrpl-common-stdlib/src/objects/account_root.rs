...
// The `AccountRoot` struct and its `AccountRootFields`/`LedgerObjectCommonFields` impls are
// generated — see `crate::objects::generated`.
pub use crate::objects::generated::AccountRoot;
...
pub fn get_account_balance(account_id: &AccountID) -> host::Result<Option<Amount>> {
    ...
    // Get the balance. `AccountRootFields::get_balance` returns a required `Amount`, but this
    // helper preserves its historical `Option` signature by calling the optional accessor
    // directly rather than going through the generated (non-optional) trait method.
    ledger_object::get_field_optional(slot, sfield::Balance)
}
