...
pub trait AccountRootFields: LedgerObjectCommonFields {
    ...
    /// hexadecimal. Must be exactly 33 bytes, with the first byte indicating the key type: `0x02`
    /// or `0x03` for secp256k1 keys, `0xED` for Ed25519 keys.
    fn message_key(&self) -> Result<Option<PublicKeyBlob>> {
        ledger_object::get_field_optional(self.get_slot_num(), sfield::MessageKey)
    }

    /// A transfer fee to charge other users for sending currency issued by this account to each
    /// other.
    ...
    /// How many total of this account's issued non-fungible tokens have been burned. This number is
    /// always equal or less than `MintedNFTokens`.
    ...
    /// The ID of the `Vault` entry associated with this account. Set during account creation;
    /// cannot be modified. If present, indicates that this is a special Vault pseudo-account
    /// AccountRoot; always omitted on non-Vault accounts.
