...
impl MPTokenIssuance {
    ...
    /// Loads the MPTokenIssuance ledger object identified by the given keylet arguments,
    /// caching it in a host-managed slot.
    pub fn load(owner: &AccountID, seq: u32) -> Result<Self> {
        let keylet = match crate::keylets::mpt_issuance_keylet(owner, seq) {
            Result::Ok(k) => k,
...
mod tests {
    ...
    fn load_success() {
        let mut mock = MockHostBindings::new();
        mock_mpt_issuance_keylet_success(&mut mock);
        mock_cache_ledger_obj_success(&mut mock, 7);
        let _guard = setup_mock(mock);

        let result = MPTokenIssuance::load(&sample::account_id(), sample::seq());
        assert!(result.is_ok());
    }
