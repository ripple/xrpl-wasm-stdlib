...
pub trait MPTokenIssuanceFields: LedgerObjectCommonFields {
    ...
    /// scale of _2_, then 1 unit of that MPT would equal 0.01 US Dollars. This indicates to how
...
mod tests {
    ...
    fn read_all_fields() {
        ...
        assert!(obj.outstanding_amount().is_ok());
        assert!(obj.previous_txn_id().is_ok());
        assert!(obj.previous_txn_lgr_seq().is_ok());
        assert!(obj.transfer_fee().is_ok());
        assert!(obj.asset_scale().is_ok());
        assert!(obj.maximum_amount().is_ok());
    ...
    fn optional_fields_none() {
        ...
        assert!(obj.transfer_fee().unwrap().is_none());
        assert!(obj.asset_scale().unwrap().is_none());
