...
impl Delegate {
    ...
    /// Loads the Delegate ledger object identified by the given keylet arguments,
    /// caching it in a host-managed slot.
    pub fn load(account: &AccountID, authorize: &AccountID) -> Result<Self> {
        let keylet = match crate::keylets::delegate_keylet(account, authorize) {
            Result::Ok(k) => k,
...
mod tests {
    ...
    fn load_success() {
        let mut mock = MockHostBindings::new();
        mock_delegate_keylet_success(&mut mock);
        mock_cache_ledger_obj_success(&mut mock, 7);
        let _guard = setup_mock(mock);

        let result = Delegate::load(&sample::account_id(), &sample::account_id_b());
        assert!(result.is_ok());
    }
