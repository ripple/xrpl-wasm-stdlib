...
pub trait LedgerHashesFields: LedgerObjectCommonFields {
    ...
    fn get_hashes(&self) -> Result<[u8; RAW_UNMAPPED_FIELD_SIZE]> {
...
mod tests {
    ...
    fn read_all_fields() {
        ...
        assert!(obj.get_hashes().is_ok());
        assert!(obj.get_first_ledger_sequence().is_ok());
        assert!(obj.get_last_ledger_sequence().is_ok());
    }
    ...
    fn optional_fields_none() {
        ...
        assert!(obj.get_first_ledger_sequence().unwrap().is_none());
        assert!(obj.get_last_ledger_sequence().unwrap().is_none());
    }
}
