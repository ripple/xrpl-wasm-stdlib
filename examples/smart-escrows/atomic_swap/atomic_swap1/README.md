...
This escrow enables trustless atomic swaps between two parties. Each escrow must reference its counterpart's ledger entry ID in the transaction memo. The escrow only releases funds if:

1. A valid counterpart escrow ledger entry ID is provided in the memo
...
- Extract escrow ledger entry ID from transaction metadata
  ...
  "Memos": [
    {
      "Memo": {
        ...
        "MemoData": "SECOND_ESCROW_ID_32_BYTES_HEX"
      }
    }
  ]
}
...
- `MemoData`: Use the ledger entry ID from Step 2's transaction metadata
  ...
  "Memos": [
    {
      "Memo": {
        "MemoType": "636F756E746572706172745F657363726F77",
        "MemoData": "FIRST_ESCROW_ID_32_BYTES_HEX"
      }
    }
  ]
}
...
- **Counterpart Ledger Entry ID**: 32-byte escrow ledger entry ID in transaction memo
...
1. **Memo Extraction**: Retrieves counterpart escrow ledger entry ID from transaction memo (32 bytes)
...
- **Ledger Entry ID-Based References**: Uses immutable ledger object identifiers for precise targeting

## Execution Flow Diagram

```mermaid
flowchart TD
    A["EscrowFinish Transaction Submitted<br/>(with memo containing counterpart ledger entry ID)"]
    B["Extract Memo Data<br/>(32-byte ledger entry ID)"]
