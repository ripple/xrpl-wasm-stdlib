...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let first_contents = fs::read_to_string(&result_files[0])?;
    let first_results: Results = serde_json::from_str(&first_contents)?;
    ...
    fs::write(&report_file, &unified_report)?;
...
mod tests {
    ...
    fn format_number_two_decimals() {
        ...
        assert_eq!(format_number(1_000_000.999), "1,000,001.00");
    }
