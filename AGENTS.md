...
**Publishing.** All four crates go to crates.io **versioned in lockstep** — every one carries the same version, and a release bumps all of them even if only one changed. That replaces a compatibility matrix with a single rule ("use `0.9.x` of everything"), so keep them equal; `xrpl-stdlib-test-utils` in particular _must_ match `xrpl-common-stdlib` exactly, since its mocks are generated from that crate's `HostBindings` trait and a mismatched pair won't compile. In-workspace deps between published crates carry both `version` and `path` — cargo builds against `path` and swaps to the registry range when packaging, so omitting `version` fails `cargo publish` with `all dependencies must have a version specified`. Under the 1.89 pin each crate publishes individually in dependency order (macros → common → test-utils → escrow; test-utils depends only on common); `cargo publish --workspace` needs 1.90. Releases are automated by `.github/workflows/publish.yml` (trigger: a `v*` tag; crates.io Trusted Publishing via OIDC, no token secret; a tag-vs-manifest guard enforces the lockstep invariant; a GitHub Release with auto-generated notes stands in for a hand-maintained changelog while pre-1.0). The maintainer-facing sequence is in `CONTRIBUTING.md`'s "Release Process". The `deprecated/xrpl-wasm-stdlib` signpost is a one-time manual publish, excluded from that workflow.
