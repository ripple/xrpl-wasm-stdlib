...
Open GitHub issues are indexed for agents in [`docs/agent-task-board.md`](docs/agent-task-board.md): claim protocol, priorities, collision groups, and which tickets already have PRs. Pick from **Ready now**, assign the GitHub issue, and do not start anything in **Do not start**. Refresh the board from GitHub if it looks stale.
