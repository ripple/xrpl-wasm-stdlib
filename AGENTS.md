...
# Regenerate from the xrpld commits pinned in .github/workflows/test.yml (requires Node.js).
# Add --check to diff without writing -- that is the check_generated CI gate.
./scripts/generate-sfields.sh          # src/sfield.rs + src/type_codes.rs
./scripts/generate-tx-flags.sh         # src/tx_flags.rs (tf*/asf*/tmf* constants)
./scripts/generate-ledger-objects.sh   # objects/generated/ (per-entry ledger-object field traits)
XRPLD_ESCROW_REF=ripple/se/supported XRPLD_CONTRACT_REF=xrplf/smart-contracts ./scripts/generate-sfields.sh --check   # against branch HEADs instead of the pins
```

Other scripts not part of the primary workflow above: `scripts/benchmark-gas.sh`, `scripts/check-wasm-exports.sh`, `scripts/docs.sh` (builds and deploys the GitHub Pages docs/UI site), `scripts/host-function-audit.sh` (see below), `scripts/run-markdown.sh`, `scripts/validate-ui.sh`, `scripts/cargo-deny.sh` (RustSec advisories on the library workspace; advisory-only in CI until the first clean run). `scripts/lib/` holds helpers the generator scripts `source` rather than standalone scripts: `xrpld-pins.sh` resolves the pinned xrpld commits from `test.yml`, `check-generated.sh` implements `--check`.
...
**Pinned generation and drift checks.** All three generators default to the xrpld commits pinned in the `env:` block of `.github/workflows/test.yml` — `XRPLD_DOCKER_IMAGE`'s tag (the `ripple/se/supported` commit the integration tests run against) and `XRPLD_CONTRACT_COMMIT` (the `xrplf/smart-contracts` commit) — resolved by `scripts/lib/xrpld-pins.sh`. `--check` on any of them regenerates in place, diffs, and restores the working tree, exiting nonzero on drift; the `check_generated` CI job and `run-all.sh` run all three, so bumping a pin without regenerating (or hand-editing generated output) fails CI, while upstream movement alone never turns a PR red. The one non-pinned input is the xrpl-dev-portal `master` pages `tools/generateLedgerObjects.js` scrapes for doc comments: a docs edit there can surface as `objects/generated/` drift. Upstream movement is tracked separately by `.github/workflows/upstream-drift.yml`: weekly (and on `workflow_dispatch`), it runs the same checks with `XRPLD_ESCROW_REF`/`XRPLD_CONTRACT_REF` set to the branch names and opens or updates an `upstream-drift` issue, closing it on the next clean run. To bump: change both pins, rerun the three scripts, and commit everything together (CONTRIBUTING.md, "Bumping the xrpld pin").
