...
- Anything new added to `HostBindings` must be implemented in all three files. CI's `scripts/host-function-audit.sh` (`tools/compareHostFunctions.js`) compares all four files (trait + three impls) against rippled's host ABI — keep them in sync. rippled's side of that ABI is Rust, not C++: `crates/xrpl-host-functions/src/lib.rs` holds the `host_functions! { ... }` declaration block (its `#[wasm_name = "..."]` attribute is the import name our bindings must match, e.g. `ldgr_index`), and `crates/xrpl-wasm-vm/src/register.rs` hand-registers each `HostFunctionSpec::<Variant>` with its real flat wasm signature. The script correlates the two by variant name (PascalCase of the declaration's fn name) and takes params/return **only** from `register.rs` — the declaration's high-level types (`&[u8]`, `i32`) don't predict wire arity (e.g. a `seq: i32` travels as a (ptr, len) region). Comparison is at the wasm-value level (`i32`/`i64`/`void`; our `*const u8`/`*mut u8`/`usize` all map to `i32`). The script fetches the live HEAD of `ripple/se/supported` with no pinned commit, so it breaks whenever upstream reshapes those two files; a genuine set difference (a function on one side only) is a real finding to report, not something to paper over in the script.
