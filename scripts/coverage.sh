...
# Coverage script for xrpl-common-stdlib
#
# This script runs the e2e-tests with coverage instrumentation to measure
# which parts of xrpl-common-stdlib are exercised by the integration tests.
#
# Requirements:
...
echo "=== xrpl-common-stdlib Coverage Report ==="
echo ""
...
# --features test-host-bindings: Enable test host bindings for xrpl-common-stdlib
...
cargo llvm-cov \
    --features xrpl-common-stdlib/test-host-bindings \
    --workspace \
    --tests \
    -- --nocapture
...
echo ""
echo "The above shows coverage for xrpl-common-stdlib exercised by e2e-tests."
echo ""
echo "To generate HTML report, run:"
echo "  cd e2e-tests && cargo llvm-cov --features xrpl-common-stdlib/test-host-bindings --workspace --tests --html --ignore-filename-regex 'e2e-tests/.*'"
echo ""
echo "To generate LCOV report, run:"
echo "  cd e2e-tests && cargo llvm-cov --features xrpl-common-stdlib/test-host-bindings --workspace --tests --lcov --output-path ../target/llvm-cov/lcov.info --ignore-filename-regex 'e2e-tests/.*'"
