...
- **`xrpl-macros`** — proc-macro crate, no runtime dependencies on the other two. Exports:
  ...
  - Entry-point macros: `#[smart_escrow]` (function-level — wraps one function in the `extern "C" fn finish()` export) and `#[smart_contract]` (module-level — scans the annotated `mod` for `#[init]`, `#[call]`, `#[user_delete]`, `#[clawback]` functions and emits one named `extern "C"` export per function; at most one `#[init]`; every export symbol in the module must be unique; lifecycle attributes export under a fixed name, `#[call]` exports under the Rust identifier by default or an explicit `#[call(name = "...")]` override — omitting `name` compiles but emits a `deprecated`-lint warning, an error under this repo's `-Dwarnings`). Both share the `parse → validate → codegen` pipeline in `entry_point/` (`ValidationRules`/`ReturnKind` are configurable per macro, e.g. the accepted wrapped return type); adding a third entry-point macro means adding a new orchestrator file there plus a new `#[proc_macro_attribute]` shim in `lib.rs`.
